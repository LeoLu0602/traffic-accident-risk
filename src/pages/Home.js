...
function Home() {
    ...
    const [centerLat, setCenterLat] = useState(22.9968);
    const [centerLng, setCenterLng] = useState(120.2169);
    ...
    const reverseGeocoding = async () => {
            try {
                const apiKey = process.env.REACT_APP_API_KEY;
                const reverseGeocodingUrl1 = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat1},${lng1}&key=${apiKey}`;
                const reverseGeocodingUrl2 = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat2},${lng2}&key=${apiKey}`;
            
                const res1 = await axios.get(reverseGeocodingUrl1);
                const res2 = await axios.get(reverseGeocodingUrl2);

                const locationArray1 = res1.data.plus_code.compound_code.split(' ');
                const districtIndex1 = locationArray1.indexOf('District,');
                const district1 = locationArray1.slice(1, districtIndex1).join(' ');
                const city1 = locationArray1[districtIndex1 + 1];

                const locationArray2 = res2.data.plus_code.compound_code.split(' ');
                const districtIndex2 = locationArray2.indexOf('District,');
                const district2 = locationArray2.slice(1, districtIndex2).join(' ');
                const city2 = locationArray2[districtIndex2 + 1];
            
                if (city1 === 'Tainan' && city2 === 'Tainan') {
                    getWeather(districtEnglish2Chinese[district1], districtEnglish2Chinese[district2]);
                }
                else if (city1 !== 'Tainan' && city2 === 'Tainan') {
                    alert('起點不在臺南市');
                } 
                else if (city1 === 'Tainan' && city2 !== 'Tainan') {
                    alert('終點不在臺南市');
    ...
    const getWeather = async (district1, district2) => {
        try {
            setIsLoading(true);
            ...
            sendRequest(district1, simplifiedWeather[weather[district1]], district2, simplifiedWeather[weather[district2]]);
        } catch (error) {
            console.error('Weather error:', error);
        }
    };
      
    const sendRequest = async (district1, weather1, district2, weather2) => {
        setSearchResults([]);

        try {
            const backendUrl1 = `https://traffic-accident-risk-backend.onrender.com/api/myfunction?district=${district1}&weather=${weather1}`;
            const backendUrl2 = `https://traffic-accident-risk-backend.onrender.com/api/myfunction?district=${district2}&weather=${weather2}`;
        
            const res1 = await axios.get(backendUrl1);
            const res2 = await axios.get(backendUrl2);
