...
function Home() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const geocodingUrl1 = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
        searchText1
      )}&key=${apiKey}`;
      const geocodingUrl2 = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
        searchText2
      )}&key=${apiKey}`;
  ...
  const reverseGeocoding = async () => {
    try {
      ...
      const pattern = /台南市.+區/;
      const match1 = res1.data.plus_code.compound_code.match(pattern)?.[0];
      const match2 = res2.data.plus_code.compound_code.match(pattern)?.[0];
  ...
  const getWeather = async (district1, district2) => {
    try {
      ...
      sendRequest(
        district1,
        simplifiedWeather[weather[district1]],
        district2,
        simplifiedWeather[weather[district2]]
      );
    } catch (error) {
